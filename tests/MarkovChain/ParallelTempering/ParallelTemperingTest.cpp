...
namespace {
    ...
    TEST(ParallelTempering, assertCorrectNumberOfProcessesAreRun) {
        ...
        hops::ParallelTempering<MarkovChainMock> parallelTempering(markovChainMock, 10, 0.00005);
        MPI_Comm TEST_COMMUNICATOR;
        MPI_Comm_dup(MPI_COMM_WORLD, &TEST_COMMUNICATOR);
        int world_size;
        MPI_Comm_size(TEST_COMMUNICATOR, &world_size);
        EXPECT_EQ(world_size, 3);
    }

    TEST(ParallelTempering, shouldProposeExchance_RngStateRemainsInSync) {
        int world_size = 3;
        MarkovChainMock markovChainMock;
        hops::ParallelTempering<MarkovChainMock> parallelTempering(markovChainMock, world_size, 0.00005);
        MPI_Comm TEST_COMMUNICATOR;
        MPI_Comm_dup(MPI_COMM_WORLD, &TEST_COMMUNICATOR);
        MPI_Comm_size(TEST_COMMUNICATOR, &world_size);
        int world_rank;
    ...
    TEST(ParallelTempering, processesAgreeOnExchangeProbability_test) {
        ...
        hops::ParallelTempering parallelTempering(markovChainMockWithColdnessAttribute, world_size, 0.5);
    ...
    TEST(ParallelTempering, processesCanExchangeState_test) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (exchangePair.first == world_rank || exchangePair.second == world_rank) {
                ...
                auto actual = parallelTempering.getCurrentState();
                ...
                auto expected = parallelTempering.getCurrentState();
