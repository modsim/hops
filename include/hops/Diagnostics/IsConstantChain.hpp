...
namespace hops {
    ...
    bool isConstantChain (const std::vector<const std::vector<StateType>*>& chains) {
        for (size_t dimension; dimension < (*chains[0])[0].size(); ++dimension) {
    ...
    template<typename StateType>
    bool isConstantChain (const std::vector<const std::vector<StateType>*>& chains, unsigned long dimension) {
        ...
        std::vector<Scalar> initDraw(numChains, 0);
	...
	for (unsigned long m = 0; m < numChains; ++m) {
	    initDraw[m] = (*chains[m])[0](d);
            ...
            for (unsigned long n = 1; n < numDraws; ++n) {
	...
	if (chainsAllConst) {
	    ...
	    // if they all equal the same constant value
            for (unsigned long m = 1; m < numChains; ++m) {
    ...
    template<typename StateType>
    bool isConstantChain (const std::vector<std::vector<StateType>>& chains, unsigned long dimension) {
        std::vector<const std::vector<StateType>*> chainsPtrArray;
        for (auto& chain : chains) {
            chainsPtrArray.push_back(&chain);
        }
